...
class OrganizationTypeController extends Controller {
     ...
     */
     public function index() {
          if (!auth()->user()->canPermission('organization_types.view')) {
          ...
          return view(
               'configurations.organization_types.index'
          );
     }
          ...
          if ($request->ajax()) {
               ...
               if (auth()->user()->canPermission('organization_types.view')) {
                    $addButtonHtml = action_button('add', [
                         ...
                         'icon' => '',
                         'color' => 'warning',
                         'createUrl' => route('organization-types.create'), 
                         'storeUrl' => route('organization-types.store'),   
                         'table' => 'organizationTypesTable'                
                    ]);
               }
               ...
               return DataTables::of($query)
                    ...
                    */
                    ->addColumn('actions', function ($row) {
                         $canEdit   = auth()->user()->canPermission('organization_types.edit');
                         $canDelete = auth()->user()->canPermission('organization_types.delete');
                         ...
                         */
                         if ($canEdit) {
                              $buttons .= action_button('edit', [
                                   ...
                                   'icon' => 'ph-pencil-line',
                                   'color' => 'primary',
                                   'editUrl' => route('organization-types.edit',$row->id),
                                   'updateUrl' => route('organization-types.update',$row->id),
                                   'table' => 'organizationTypesTable'
                              ]);
                         } else {
                              $buttons .= '
                         ...
                         */
                         if ($canDelete) {
                              $buttons .= action_button('delete', [
                                   ...
                                   'icon' => 'ph-trash',
                                   'color' => 'danger',
                                   'url' => route('organization-types.destroy',$row->id),
                                   'method' => 'DELETE',
                                   'table' => 'organizationTypesTable'
                              ]);
                         }
     ...
     */
     public function create(){
          if (!auth()->user()->canPermission('organization_types.create')) {
               return response()->json([
                    'success' => false,
                    'message' => 'Permission denied'
               ], 403);
          ...
          return view(
               'configurations.organization_types.partials.form'
          );
     }
     ...
     public function store(Request $request) {
          ...
          $validator = Validator::make(
               ...
               [
                    ...
                    'slug' => ['nullable','string','max:255','unique:organization_types,slug'],
     ...
     */
     public function edit(OrganizationType $organizationType) {
          if (!auth()->user()->canPermission('organization_types.edit')) {
               return response()->json([
                    'success' => false,
                    'message' => 'Permission denied'
               ], 403);
          ...
          return view('configurations.organization_types.partials.form',compact('organizationType'));
     }
     ...
     public function update(Request $request,OrganizationType $organizationType) {
          ...
          $validator = Validator::make(
               ...
               [
                    ...
                    'slug' => ['nullable','string','max:255','unique:organization_types,slug,' . $organizationType->id],
                    'description' => ['nullable','string'],
                    'icon' => ['nullable','string','max:255'],
                    'color' => ['nullable','string','max:50'],
                    'sort_order' => ['nullable','integer','min:0'],
     ...
     */
     public function destroy(OrganizationType $organizationType) {
          if (!auth()->user()->canPermission('organization_types.delete')) {
               return response()->json([
                    'success' => false,
                    'message' => 'Permission denied'
               ], 403);
